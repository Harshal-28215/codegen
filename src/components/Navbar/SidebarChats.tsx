...
function SidebarChats({chat,setIsOpen}:SidebarChatsProps) {
    return (
        <div className='mt-2 w-full overflow-y-auto flex items-center gap-2 hover:bg-white/20 rounded-md p-2 cursor-pointer justify-between'>
            <Link href={`/Workspace/${chat._id}`} onClick={()=>setIsOpen(false)} className='text-white overflow-hidden text-ellipsis whitespace-nowrap w-[calc(100%-24px)]'>{chat.message[0].message}</Link>
